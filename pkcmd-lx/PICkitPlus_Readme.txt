...
3.  Changes
  ...
  PICKITCOMMANDLINE 4.27       Fixed "-devicelist" causing unhandled exception

  4.61.0.56
        Now correctly writes microcontrollers with greater than 0x8000 PROGMEM.  This explicitly impacted the 18FxxQ10 range of microcontrollers.
  PICKITCOMMANDLINE 4.27       Now correctly writes microcontrollers with greater than 0x8000 PROGMEM.  This explicitly impacted the 18FxxQ10 range of microcontrollers.
                               Fix for locked log files.

  4.61.0.57
        Now correctly writes/verifies PIC24F16xx class of microcontrollers.  This class requires a WRITE operation with NO verify followed a VERIFY operation.  The software handles this automatically.  However, when using CPD this will cause an error - to resolve verify without CPD to test your code then set the CPD in your final programming activity.

  4.61.0.58
        Now correctly unlocks PKGUI UI when VERIFY was not selected

    PICKITCOMMANDLINE 4.42:
      PKEasy: Fixed inability to type in the device box
      GUIs: Now persists window size as well as position
      GUIs: Will never start entirely off the edge of the screen (how that even happens I will never know)
      GUIs: Fixed InvalidCastException when Override Voltage is checked.
      GUIs: Now expect their dat file in "%LocalAppData%\Infinity Computers\<program name>\" and will prompt to install it if necessary
      GUIs: Now update their parts lists when a new dat file is downloaded
      GUIs: Removed all auto-update functionality.
      GUIs: Now expect their dat file alongside the exe, as before
      GUIs: Moved voltage setting to toolbar, and changed to a menu of presets
      GUIs: Added indication to "Apply Power" button
      GUIs: May now apply power without a selected part
      GUIs: Renamed "Provide Power" to "Provide Power during programming"
      GUIs: May now apply power without enabling "power during programming"
      GUIs: Programming power options now greyed when power is manually applied
      GUIs: Manually-applied power now remains after programming, except when there is an error
      GUIs: Fixed Get<Decimal>(PowerOverrideValue) exception
      Easy: Fixed appearance of mute button
      GUIs: Fixed odd (actual) voltage being displayed
      GUIs: Fixed flicker when changing active voltage
      GUIs: Clicking "Apply Power" does nothing if it is already selected
      GUIs: Displayed voltage when applying power will now be correct
      Easy: Added About dialog
      Easy: Added BuildDate resource
      Fixed inability to detect a chip (regression introduced in 4.34, by calling READ_STATUS during a PROG operation)
      GUIs: Don't cut power when beginning a programming operation
      Fixed --firmware regression due to CutPower() being called in Main()
      Fixed bandgap output
      Fixed bug where only 128 bytes were read from the target part
      Added slight delay after applying power, to avoid low VDD errors

  5.61.0.60
      Support for 18fxxQ43 added - uses reverse Config Addressing

  5.61.0.61
      Resolve MCRL check box selection issue

  5.61.0.62
      Implemented DPI handler for Hi-Res displays

  5.61.0.63
      Support for 18fxxQ41 added - uses reverse Byte Config Addressing
      Support for alternative ICONs

  5.61.0.64
      EE checkbox fix and UI changes in ConfigEdit for 18FxxQ series
  ...
  5.61.0.66
      Support for 18fxxQ40 added - uses

  6.61.0.67
      Turbo mode added

  6.61.0.68
      Corrected issue where the VDD was left On where erasing with HEF/SAF selected.
